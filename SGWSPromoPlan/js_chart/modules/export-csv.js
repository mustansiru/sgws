            ...
            if (series.options.includeInCSVExport !== false) {
                ...
                each(series.points, function (point) {
                    ...
                    if (window.location.href.indexOf("report.aspx?Path=RFID+EM+Display+Compliance+Reports%2fPerformance+By+Geography&type=1") > -1) {
                        ...
                        { if (i == 1) { rows[point.x][i - 1] = point.Y + '%'; } else { rows[point.x][i] = '-'; } }
                        else {
                            if (point.value == 0)
                                rows[point.x][i] = '-';
                            else
    ...
    Highcharts.Chart.prototype.getCSV = function (useLocalDecimalPoint) {
        ...
        if (window.location.href.indexOf("report.aspx?Path=RFID+EM+Display+Compliance+Reports%2fPerformance+By+Geography&type=1") > -1) {
            ...
            if (this.title != undefined) {
                rowtitle1.push(this.title.textStr);
            }
        ...
        rowtitle1 = [];
        rowtitle1.push(($.trim($("#divReportGenarationTitle").text())).replace(/\s\s+/g, ' '));
        rowtitle1.push('');
        rowtitle1.push(lineDelimiter);
        csv += rowtitle1.join(itemDelimiter);
        rowtitle1 = [];
