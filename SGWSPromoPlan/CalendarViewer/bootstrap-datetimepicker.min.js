...
var highlightMonths = [];
var toHighlightYear = [];
var toHighlightMonth = [];
var toHighlightMonth_by_Name = [];
var Months_by_Name = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']

//$(function () {

//    $.ajax({
//        method: "GET",
//        url: "/BCCService.asmx/Get_Locked_Months?CompanyID=" + companyID_monthLocked,
//        processData: false,
//        async: true,
//        success: function (data) {
//            highlightMonths = data.split(',');
//            $("span[data-action='selectMonth']").removeClass('highlight');
//        },
...
function SetHighlight() {
    ...
    toHighlightYear = [];
    toHighlightMonth = [];
    toHighlightMonth_by_Name = [];

    var items = [];
    $("table thead th[class='picker-switch']").each(function (yearIndex, year) {
        if (yearIndex == 1)// month view
        {
            highlightMonths.forEach(function (item, index, arr) {
                items = item.split('-');
                if (items.length > 0) {
                    if ($(year).html() == items[0]) {
                        if ($.inArray(items[0], toHighlightYear) == -1) {
                            toHighlightYear.push(items[0]);
                        }
                        if ($.inArray(items[1], toHighlightMonth) == -1) {
                            toHighlightMonth.push(items[1]);
                        }
                    }
                }
            });
            if ($.inArray($(year).html(), toHighlightYear) >= 0) {
                toHighlightMonth.forEach(function (item, index) {
                    if ($.inArray(Months_by_Name[parseInt(item) - 1], toHighlightMonth_by_Name) == -1) {
                        toHighlightMonth_by_Name.push(Months_by_Name[parseInt(item) - 1]);
                    }
                });

                $($($($(this).parent()).parent()).parent()).find("span[data-action='selectMonth']").each(function () {
                    if ($.inArray($(this).html(), toHighlightMonth_by_Name) >= 0) {
